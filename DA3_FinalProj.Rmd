...
Code Housed in [GitHub Repository](https://github.com/blg192/Data3Project3) 
...
```{r clusterplot1, echo = FALSE, results='asis', fig.cap = '\\label{fig:SSTClusters} Best clustering solutions for each of the dimension reduction methods for SST data.', fig.align = 'center', fig.height=8, fig.pos="H"}
## SVD plot
...
```{r clusterplot2, echo = FALSE, results='asis', fig.cap = '\\label{fig:PrecipClusters} Best clustering solutions for each of the dimension reduction methods for Precipitation data.', fig.align = 'center', fig.height=7.5, fig.pos="H"}
## SVD plot
...
It is well known by experts in the field that one of the most simple models - the climatology model - performs relatively well given the ease of calculation and interpretation. The climatology model simply takes historical average for a given month and uses that as the prediction for that month in the future. Two climatology models were built with this data: one using the continuous precipitation values and one using the categorical precipitation values. The MSE of the model with the continuous values was 1.494, whereas the accuracy for the categorical values was 0.712. These values would need to be improved by other models described below to be considered as a replacement.
...
```{r pers_mod_plot, echo=FALSE, cache=TRUE, out.width="70%", fig.align='center'}
...
```{r ridge1, echo=FALSE, cache=TRUE, eval=FALSE}
# load 
load("SST_clustwide.RData")
load("Pdat_clustwide.RData")
load("SST_longagg.RData")
...
precip_yyhat2 <- data.frame(all_long_test[, c(1, 7:8)], "yhat" = ridge_pred)
for(i in 1:nrow(precip_yyhat2)){
  if(precip_yyhat2$landclus[i] == "LandCluster1"){
    precip_yyhat2$landclus[i] <- 1
  } else if(precip_yyhat2$landclus[i] == "LandCluster2"){
    precip_yyhat2$landclus[i] <- 2
  } else if(precip_yyhat2$landclus[i] == "LandCluster3"){
    precip_yyhat2$landclus[i] <- 3
  } else if(precip_yyhat2$landclus[i] == "LandCluster4"){
    precip_yyhat2$landclus[i] <- 4
  } else if(precip_yyhat2$landclus[i] == "LandCluster5"){
    precip_yyhat2$landclus[i] <- 5
  } else if(precip_yyhat2$landclus[i] == "LandCluster6"){
    precip_yyhat2$landclus[i] <- 6
  } else if(precip_yyhat2$landclus[i] == "LandCluster7"){
    precip_yyhat2$landclus[i] <- 7
  } else if(precip_yyhat2$landclus[i] == "LandCluster8"){
    precip_yyhat2$landclus[i] <- 8
  } else if(precip_yyhat2$landclus[i] == "LandCluster9"){
    precip_yyhat2$landclus[i] <- 9
  } else if(precip_yyhat2$landclus[i] == "LandCluster10"){
    precip_yyhat2$landclus[i] <- 10
  } else if(precip_yyhat2$landclus[i] == "LandCluster11"){
    precip_yyhat2$landclus[i] <- 11
  } else{
    precip_yyhat2$landclus[i] <- 12
  }
}
precip_yyhat2$landclus <- as.factor(precip_yyhat2$landclus)

## limit long_Pdat to the test observations
long_Pdat_test <- long_Pdat[min(which(long_Pdat$Date == "Jan2017")):nrow(long_Pdat),]

all_precip_yyhat2 <- merge.data.frame(long_Pdat_test, precip_yyhat2, 
                                     by.x = c(4, 3), by.y = 1:2)

## save data
save(all_precip_yyhat2, file = "all_precip_yyhat2.RData")
...
```{r lasso1, echo=FALSE, cache=TRUE, eval=FALSE}
...
precip_yyhat3 <- data.frame(all_long_test[, c(1, 7:8)], "yhat" = lasso_pred)
for(i in 1:nrow(precip_yyhat3)){
  if(precip_yyhat3$landclus[i] == "LandCluster1"){
    precip_yyhat3$landclus[i] <- 1
  } else if(precip_yyhat3$landclus[i] == "LandCluster2"){
    precip_yyhat3$landclus[i] <- 2
  } else if(precip_yyhat3$landclus[i] == "LandCluster3"){
    precip_yyhat3$landclus[i] <- 3
  } else if(precip_yyhat3$landclus[i] == "LandCluster4"){
    precip_yyhat3$landclus[i] <- 4
  } else if(precip_yyhat3$landclus[i] == "LandCluster5"){
    precip_yyhat3$landclus[i] <- 5
  } else if(precip_yyhat3$landclus[i] == "LandCluster6"){
    precip_yyhat3$landclus[i] <- 6
  } else if(precip_yyhat3$landclus[i] == "LandCluster7"){
    precip_yyhat3$landclus[i] <- 7
  } else if(precip_yyhat3$landclus[i] == "LandCluster8"){
    precip_yyhat3$landclus[i] <- 8
  } else if(precip_yyhat3$landclus[i] == "LandCluster9"){
    precip_yyhat3$landclus[i] <- 9
  } else if(precip_yyhat3$landclus[i] == "LandCluster10"){
    precip_yyhat3$landclus[i] <- 10
  } else if(precip_yyhat3$landclus[i] == "LandCluster11"){
    precip_yyhat3$landclus[i] <- 11
  } else{
    precip_yyhat3$landclus[i] <- 12
  }
}
precip_yyhat3$landclus <- as.factor(precip_yyhat3$landclus)

## limit long_Pdat to the test observations
long_Pdat_test <- long_Pdat[min(which(long_Pdat$Date == "Jan2017")):nrow(long_Pdat),]

all_precip_yyhat3 <- merge.data.frame(long_Pdat_test, precip_yyhat3, 
                                     by.x = c(4, 3), by.y = 1:2)

## save data
save(all_precip_yyhat3, file = "all_precip_yyhat3.RData")
...
The regularization method in this study will use the clustering for land and sea resulted from KPCA. From the KPCA, the most appropriate results suggest five clusters for SST data and twelve clusters are appropriate for Precipitation data. We used the average precipitation data in given month_year as response and average SST for each sea cluster in given_month as predictors. We set the training set using all the data before 2017 (Jan1948 to Dec2016) and testing set using all data after 2017 (Jan2017 to Feb2018).
The Ridge regression produce the coefficient estimates that fit the data well by making the RSS small and has the effect of shrinking the estimates toward zero. The ten-fold cross validation is used to choose the tuning parameter $\lambda$. The value of $\lambda$ that results in the smallest cross validation error is 3.15. The test RMSE for Ridge Regression is 4.3320.

The Lasso Regression has similar behavior to the ridge regression. Cross validation was used to find the best $\lambda$ at 0.0116. The test MSE for the Lasso Regression is 4.3562. Both Ridge and Lasso regression produce test MSEs which are not significantly different. Even though the test MSE for lasso regression is a bit higher than ridge regression, the lasso performs variable selection like best subset selection and it can be more interpretable than ridge regression. From the resulted test MSE, it seems that the use of ridge and lasso regression is not the most effective way to predict precipitation data. However, ridge regression and lasso regression can be more computationally feasible than subset selection method.
...
gam.1<- gam(Precipitation~s(SeaCluster1,2) +SeaCluster2 + SeaCluster3 + SeaCluster4 + SeaCluster5, data = Lag.dat)

```

The generalized linear approach proved to be less than ideal. The idea of approaching the data using GAM was the nonlinear relationships between the individual clusters and the precipitation data would be modeled while maintaining the additive nature of a linear model. Using the KPCA regions with continuous data as the response, we began by running the GAM model without adjusting for nonlinearity for each SST cluster, to find that all of the variables were significant linearly, though barely so for SeaCluster3. Knowing this, we wanted to experiment with the nonlinearity between some of the clusters and the response, therefore we opted for smoothing splines on the first two SST clusters with degrees of freedom 2. SeaCluster1 and SeaCluster2 both proved to be significant at with 2 degrees of freedom. 

First we generate a model which predicted the precipitation at a given time $\tau$ given the SST clustered data at the same time $\tau$. The GAM model proved to predict precipitation levels using contemporary SST data with very low accuracy. Against the 2017/18 test data, the best GAM model produced an MSE of 8.5, with a smoothing spline on the first cluster. We then built a separate model using the same KPCA clusters at a given time $\tau$ given the SST clustered data 6 months prior, i.e. $\tau$+6 months. This lagged data gives us more of an idea on how sea surface temperature has effects over time, and while the model still performed poorly with an average MSE across clusters of 6.312, it did perform significantly better than the contemporary data.
...
```{r, include = TRUE, out.width = "70%", fig.align='center', echo = FALSE}
...
Although we had some models with low test MSE and fair to good accuracy, we were not able to beat the climatology baseline. The CNN models generally performed the best out of our models, with the random forest using the lagged data falling in between two of the CNN models' test MSE values.

One issue that arose in the CCN analysis using the categorical precipitation was that the majority of locations have "normal" precipitation levels. Even when considering different class weights to account for the differences, it was difficult for the model to accurately predict the true categories. Future analysis could attempt to improve these results by investigating different values of $\tau$ in the lag models and devising a different methodology for categorizing precipitation, such as using different quantile values as the boundaries or using a method other than quantiles for categorizing the data.
